<template>
    <div class="snPopupPicker">
        ...
        <div v-else>
            <cell :title="title+(titleIndex?titleIndex:'')" value-align="left" class="snPopupRight" @click.native="openPopup">
                <input type="text" class="popupInput sn-pointer" maxlength="20" :placeholder="placeholder" readonly="readonly" ref="input" :value="selectedValue">
            </cell>
            <div v-transfer-dom>
...
export default {
    ...
    created: function () {
        var _this = this;
        if (_this.selectType == "debitcard") {
            //取结算卡列表数据
            //从服务器获取结算卡的数据
            _this.getDebitcardList();
        } else if (_this.selectType == "payerAccount") {
            //密码器模板 付方账号     后续需要优化
            //从服务器获取密码器模板 付方账号
    ...
    mounted: function () {
        ...
        var _this = this;
        _this.dataList = JSON.parse(JSON.stringify(_this.list));
    ...
    methods: {
        ...
        choosedItem: function (item) {
            ...
            setTimeout(() => {
                ...
                if (_this.selectType == "branch") {
                    //如果是支行，则在content中的存储branchDisplay来显示
                    _this.setContentValue(item.value, item.dispValue);
                }
        ...
        /**
         * 设置父组件content中的值
         * @param {Object} branchId   数据中的branchId
         * @param {Object} branchDisplay 数据中的branchDisplay
         */
        setContentValue: function (branchId, branchDisplay) {
            var _this = this;
            var branch = { 'branchId': branchId, 'branchDisplay': branchDisplay };
            _this.$emit('branch', branch);
        },
        /**
         * 查询结算卡数据
         */
        getDebitcardList: function () {
            var _this = this;
            var debitcardList = [];
            cryptPost("debitcard/queryCpyDebitCardList.html", {}).then(function (
                result
            ) {
        ...
        showBranch: function () {
            ...
            var branchList;
            var branchDataList;
            var bankId = _this.bankId;
            var cityId = _this.cityId;
            if (!cityId || "" == cityId) {
                showToast("请先选择开户地区");
                return false;
            }
            doPost("openAccount/downloadBranchBankList.do", {
                bankId: bankId,
                cityId: cityId,
                pageNum: 100,
        ...
        showorderTimeId: function () {
            ...
            var week = parseInt(
                _this.getWeek(new Date(_this.orderDate * 1000).format("yyyy-MM-dd"))
            );
            doPost("openAccount/getBranchPeriodInfo.do", {
                branchId: branchId
            }).then(function (result) {
                if (0 == result.code) {
                    //根据已选银行，省市，请求支行
                    _this.periodList = result.data.periodList;
