    ...
    <div class="wealth-account">
        ...
        <div v-if="!pageLoading">
            ...
            <div>
                ...
                <div class="profit-card"  v-if="!noInfo" v-for="(finance,i) in financeList" :key='i' @click="redeemWealth(finance)">
...
import {openRouterByValue,openNativePage,backRouterByValue,getBusinessData,setBusinessData,getServerData,formatProduct,getSignData,getServerDataH5 } from '../../handler/handler'
    ...
    methods:{
        ...
        init(){
            ...
            if(!!_this.$route.query.phone){
                ...
                _this.sessionKey = 'financeData'+_this.loginData.UAId;
            }
            _this.queryFinancialPosition();
            // if(!!_this.$route.query.tradeFlag){
            //     if(!!getBusinessData(_this.sessionKey)){
            //         _this.financeList = getBusinessData(_this.sessionKey).financeList;
            //         _this.totalAmount = getBusinessData(_this.sessionKey).totalAmount;
            //         _this.totalInterest = getBusinessData(_this.sessionKey).totalInterest;
            //     }else{
            //         _this.noInfo = true;
            //     }
            // }else{
        ...
        async queryFinancialPosition(){
            ...
            try {
                ...
                const result = await getServerDataH5(textUrl+'/yqt/wealth/wealthManage.queryFinancialPosition',data);
                console.log(result)
                ...
                if(result.code === 0){
                    ...
                    if(!!result.financeList&&result.financeList.length){
                        ...
                        setBusinessData(_this.sessionKey,{financeList:_this.financeList,totalAmount:_this.totalAmount,totalInterest:_this.totalInterest});
                    }else{
                        _this.noInfo = true;
                    }
                }else{
        ...
        showRecord(){
            let _this = this;
            openRouterByValue('traderecord',{phone:phone,},_this);
        },
        /**
         * 理财赎回
         */
        redeemWealth(finance){
            let _this = this;
            openRouterByValue("redeemdetail",{phone:phone,finance:encodeURIComponent(JSON.stringify(finance))},_this);
        }
    }
}
</script>
