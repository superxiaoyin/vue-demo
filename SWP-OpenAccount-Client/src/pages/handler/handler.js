 ...
 * 获取服务器返回数据(H5)
 */
export function getServerDataH5(url,data,type,tfyType){
    return new Promise((res,rej)=>{
        if(!!data.phone){
        ...
        //天府云登录信息获取
        if(!!tfyType && !!data.UAId){
            let tmpData = getBusinessData('userData'+data.UAId);
            if(!!tmpData){
                if(!!tmpData.UAId){
                    let seq = Common.newSeq();
                    data.UAId = tmpData.UAId;
        ...
        Common.AjaxFun(type||'POST',url, data, function (result) {
            if(!!result.success){
 ...
 *  路由跳转页不传this
 * */
export function newOpenRouterByValue(name,value){
    if(value||JSON.stringify(value)!=='{}'||value.length){
...
export const wealthMap= {
    '月月红':[{title:'产品亮点',value:'一次性存入本金，每月取息',iconUrl:'../resource/img/wealth/icon_light.png'},{title:'发行单位',value:'泸州银行',iconUrl:'../resource/img/wealth/icon_unit.png'},{title:'产品概述',value:'一次性存入本金，每月取息',iconUrl:'../resource/img/wealth/icon_summary.png'},{title:'存款规则',value:'每月领取利息=本金*年利率*存期(5年)/总月数(60个月)，次月开始领取利息，每月自动返息到指定账户。',iconUrl:'../resource/img/wealth/icon_state.png'},{title:'销售状态',value:'在售',iconUrl:'../resource/img/wealth/icon_state.png'}],
