...
<script>
import Vue from 'vue';
import { appList,getServerData,setBusinessData,getBusinessData,deleteBusinessData,formatProduct,openNativePage, openRouterByValue,setLocalData,getLocalData} from '../handler/handler'
...
export default {
    ...
    methods:{
        ...
        init(){
            let _this = this;
            if(!!phone){
                _this.loginData = getLocalData(phone);
            }
        },
        /**
         * 打开认证页面
         ...
         * 检查2类开户条件
         */
        checkOpenAccount(url){
            let _this = this;
            let param = {
                UAId:_this.loginData.UAId,
                lgParam:_this.loginData.lgParam
            }
            try {
                //第一步查询认证系统工作时间
