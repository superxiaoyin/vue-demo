...
import { getServerData,openRouterByValue,formatProduct,setBusinessData,getBusinessData,getServerDataH5 } from '../../handler/handler'
...
export default {
    data(){
        return{
            productList:[],
            pageLoading: false,
            subTitle:[this.$route.query.p_family == 1 ? '快乐金' : '月月红'],
            noInfo:false,
    ...
    methods:{
        ...
        init(){
            ...
            if(!!_this.$route.query.phone){
                ...
                if(!!_this.loginData){
                    _this.sessionKey = 'wealthData'+_this.loginData.UAId;
                }
            }else if(!!phone){
            ...
            //     _this.productList = getBusinessData(_this.sessionKey);
            // }else{
            //     _this.queryWealthList();
            // }
        },
        ...
        showWealthDetail(product){
            let _this = this;
            openRouterByValue('wealthdetail',{phone:phone,product:encodeURIComponent(JSON.stringify(product)),listFlag:1,p_family:_this.pFamily},_this);
        },
         /**
         * 查询财富产品列表
         */
        async queryWealthList(){
            ...
            const data = {
                ...
                businessType: 1,
                pType: 1,
                pFamily:_this.pFamily, // 1是快乐金，2是月月红
                phone:phone,
                channel:2
            }
            try {
                _this.pageLoading = true;
                const result = await getServerDataH5(textUrl+'/yqt/wealth/wealthManage.queryProductListForH5',data);
                ...
                if(result.code === 0){
                    if(!!result.productList&&result.productList.length){
                        ...
                        setBusinessData(_this.sessionKey,_this.productList);
                    }else{
                        _this.noInfo = true;
                    }
                }else{
