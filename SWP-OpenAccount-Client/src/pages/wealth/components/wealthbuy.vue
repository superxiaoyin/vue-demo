<template>
    <div class="wealth-buy">
        ...
        <div class="product-input">
            ...
            <div class="product-tip" v-if="pFamily == 1">
                <sn-cell title="年利率">{{product.rate|percentFrt}}</sn-cell>
                <sn-cell :title="`${num2Chinese(product.dueTime)}年累计利息`">{{!!yearInterest?yearInterest:yearInterest|moneyFrt(1)}}</sn-cell>                
            </div>
        </div>
        <div class="buy-btn">
            <x-button type="primary" :show-loading="payLoading"  @click.native='confirmPay' :disabled="payLoading" style="border-radius:99px;line-height: 0.84rem;background: #E8422E;font-size: 0.32rem;">{{payText}}</x-button>
...
import { getServerData,openNativePage,getSignData,openRouterByValue,backRouterByValue,num2Cn,formatSelectList,getBusinessData,setBusinessData,getServerDataH5,getSignDataH5 } from '../../handler/handler'
import { SnCell } from 'components'
...
export default {
    ...
    components:{
        SnMoney,SnCell,XButton,Popup,Panel
    },
    computed:{
        interestDesc(){
    ...
    methods:{
        ...
        confirmPay(){
            ...
            if(errorCode== 0){
                ...
                getServerDataH5(textUrl+'/yqt/wealth/wealthManage.buyFinancialProduct',payData,false).then(res=>{
                    console.log(res)
                    let payResult = res
                    _this.payLoading = false;
                    _this.payText = "确定";
                    ...
                    if(payResult.code === 0 && !!payResult.voucherNo){//支付成功
                        ...
                        tmpdata.status = 1;
                        tmpdata.text = '购买成功';
                        tmpdata.amount = Number(_this.buyAmount);
                        tmpdata.infoList = [
         ...
         * 计算利息(快乐金)
         */
        calculateInterestGold(amount,year,yearRate){
            let _this = this;
            const rate = _this.mathDivide(yearRate,100000);
            let monthMate = _this.mathDivide(rate,12);
            let totalMonth = _this.mathMultiply(year,12);
            let totalMate = _this.mathMultiply(_this.mathDivide(_this.mathMultiply(_this.mathAdd(totalMonth,1),totalMonth),2),monthMate);
            let yearInterest = _this.mathMultiply(amount,totalMate);
            return Number(math.round(yearInterest,2)).toFixed(2)
        }
    },
    watch:{
        buyAmount(newVal,oldVal){
            let _this = this;
            if(!!newVal&&_this.pFamily==2){
            ...
            }else if(!!newVal&&_this.pFamily==1){
                _this.yearInterest = _this.calculateInterestGold(_this.mathDivide(newVal,100),_this.product.dueTime,_this.product.rate);
            }else{
