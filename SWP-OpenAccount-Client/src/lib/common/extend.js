...
import {
    bankMap
} from '../../lib/ConstantData.js';
import {
    imgBase64Map
} from './imgBase64Map.js';
import {
    themeColor
} from '../../components/style/themeconfig';
import {
    doPost
} from "../../lib/common/Net.js";
...
export function showConfirm(content, rightFunction, type, strLeftBtn, strRightBtn, title, leftFunction, H5Flag) {
    ...
    if (!H5Flag) { //非H5方法
        ...
        var confirmJson = { //调用native弹框方法请求参数
            ...
            rightBtnFontColor: themeColor || '#478aee', //右边按钮默认颜色
...
export function getBankConfig() {
    return doPost('common/getBankConfig.do', {}).then(function (result) {
        if (!!result && result.data && !!result.data.bankInfo) {
            let bankconfig = result.data.bankInfo[0];
...
Vue.filter('percentFrt', function (percent) {
    if (percent == 0) {
        return 0;
    }
    let str = Number(percent / 1000).toFixed(3);
    str += "%";
    return str;
});
...
/**
 * @description: 保管箱租赁租期周期
 * @param : 
 * @return: 
 */
Vue.filter('dateUnitFrt', function (data) {
    let dateUnit = [{
            name: "周",
            value: "W"
        },
