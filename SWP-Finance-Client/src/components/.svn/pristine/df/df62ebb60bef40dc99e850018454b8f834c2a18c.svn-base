        ...
        methods: {
			...
			downLoadFileReq:function(item, index){
				...
				CheckNetWork(function(){
					...
					MstpFileHandler.downloadFileUrl(fileId).then(function(dlUrl){
                        ...
                        if (isPC()) {
                            $("#downloadhref").attr("href", previemJson["downloadUrl"]);
            ...
            getPreviewAttachList:function(attachDetailList){
                AttachHandler.formatDetailForPreview(attachDetailList).then(function(previewAttachList){
