    ...
    <cell :title="title" class="sn-textarea" value-align="left">
        <div class="imgupWrap" :data-num="datanum">
            <div v-for="(item, index) in previewAttachList">
                <div class="imgDiv">
                    <img
                        v-imgfid="item.imgpic"
                        :src="item.imgsrc"
                        @click="downLoadFileReq(item, index)"
                    />
                </div>
            </div>
            <div
                ...
                @click="selFileBt(1)"
            >
                <!--selFlg==1：上传按钮显示，其他情况不显示-->
                <a class="attachA" href="javascript:;" :id="imageUploadId"></a
...
export default {
    ...
    mounted: function() {
        ...
        if (_this.value.length > 0) {
            //草稿内容读取
            _this.getPreviewAttachList(_this.value);
            if (_this.isInDetailFlag) {
                //明细
                _this.seUploadFileList =
                    _this.$root.content.inDetail[_this.idx].imageUpload_final;
            } else {
        ...
        doPost('../common/getMSTPConfig.do', param, null, true).then(function (data) {
                console.log(data);
	        		if (!!data) {
                    
						MstpConf.ipAddress = data.paramValue.MSTP_FS_ipAddress;
						MstpConf.previewAddress = data.paramValue.MSTP_FS_previewAddress;
						res(null);
	        		}
	        	});
    },
    ...
    watch: {
        value: function(oldVal, newVal) {
            //主要供明细控件删除使用
            var _this = this;
            if (_this.$parent.delInDetailFlag) {
                //明细控件删除
                //判断value类型
    ...
    methods: {
        ...
        addBeforeFunction: function(_this, files) {
            CheckNetWork(function() {
                ...
                var msg = AttachHandler.formatApplyForPreview(
                    _this.previewAttachList,
                    files,
                    _this.fjAllCount,
                    fCount
        ...
        addSuccessFunction: function(_this, file, fileId) {
            ...
            for (var i = 0; i < _this.previewAttachList.length; i++) {
                ...
                if (file.transactionId == fileDetailObj.transactionId) {
                    ...
                    var seUpObjkey =
                        "imageUpload_yuyue_" + this.datanum + "_" + seIndex;
                    var seUpObjValue = {
        ...
        downLoadFileReq: function(item, index) {
            ...
            CheckNetWork(function() {
                ...
                MstpFileHandler.downloadFileUrl(fileId)
                    .then(function(dlUrl) {
                        return new Promise(
                            function(res, rej) {
                                previemJson["downloadUrl"] = dlUrl;
                                res();
                            },
                            function() {
                                rej();
                    ...
                    .then(function() {
                        return MstpFileHandler.previewFileUrl(fileId);
                    })
                    .then(function(previewUrl) {
                        console.log("downLoadFileReq  previewUrl", previewUrl);
                        previemJson["previewUrl"] = previewUrl;
                        if (isPC()) {
                            $("#downloadhref").attr(
                                "href",
                                previemJson["downloadUrl"]
                            );
                            ...
                            document
                                ...
                                .dispatchEvent(ev); //pc不支持要click()方法，要自己实例化一个事件对象，然后模拟触发
        ...
        refreshImgList: function() {
            ...
            // });
            _this.getPreviewAttachList(_this.seUploadFileList);
        },
        /**
         * 判断附件是否全部上传成功
         */
        ...
        getPreviewAttachList: function(attachDetailList) {
            AttachHandler.formatDetailForPreview(attachDetailList).then(
                function(previewAttachList) {
                    _this.previewAttachList = previewAttachList;
                    _this.previewAttachList.forEach(function(item, index) {
                        item["delFg"] = "1"; //读取草稿的时候需要显示删除按钮
                        _this.previewAttachList[index] = item;
                    });
