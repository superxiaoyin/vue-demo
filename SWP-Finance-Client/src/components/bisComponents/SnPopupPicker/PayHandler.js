 ...
 * 支付相关方法
 */
import { verifySignFunction } from '../../../lib/common/SnJsBridge.js';
import { cryptPost } from '../../../lib/common/base.js';
import {transArrToStr,moneyStringFixTwo,showToast} from '../../../lib/common/extend.js';
 ...
 */
const genSignMap = {
	[PayTypeData.TRANSFER_TYPE] : {url:'yqt/pay/getSign.html'},//转账
	[PayTypeData.SALARY_TYPE] : {url:'yqt/pay/getSign.html'},//代发工资 后续会删除
	[PayTypeData.WEALTH_BUT_TYPE] : {url:'yqt/pay/getSign.html'},//财富管理 购买 后续会删除
	[PayTypeData.WEALTH_REDEEM_TYPE] : {url:'yqt/pay/getSign.html'},//财富管理 赎回 后续会删除
	[PayTypeData.BALANCE_TYPE] : {url:'yqt/pay/getSign.html'},//银企对账 后续会删除
	[PayTypeData.DSS_TYPE] : {url:'yqt/pay/getSign4Bill.html'},//票据
	[PayTypeData.DSS_TEMPLET_TYPE] : {url:'yqt/pay/getSign4Bill.html'},//安全硬件模板
	[PayTypeData.CREDCARD_TYPE] : {url:'yqt/pay/credcard_getSign.html'},//结算卡
	[PayTypeData.DIGITALCHECK_TYPE] : {url:'yqt/pay/digital_getSign.html'},//数字支票
	[PayTypeData.BATCHTRANSFER_TYPE] : {url:'yqt/pay/getSign.html'}//批量转账
};
...
const verifySignMap = {
	...
	*/
	...
	[PayTypeData.DSS_TEMPLET_TYPE] : {fnt:'CalculatePasswordFunction'},//安全硬件模板
 ...
 */
const endFlowMap = {
	[PayTypeData.TRANSFER_TYPE] : {url:'yqt/pay/transfer.html'},//转账
	[PayTypeData.SALARY_TYPE] : {url:'yqt/pay/sendSalary.html'},//后续会删除  创建工资流程
	[PayTypeData.WEALTH_BUT_TYPE] : {url:'yqt/wealth/buy.html'},//后续会删除  购买理财
	[PayTypeData.WEALTH_REDEEM_TYPE] : {url:'yqt/wealth/redeem.html'},//后续会删除 赎回理财
	[PayTypeData.BALANCE_TYPE] : {url:'yqt/balance/balanceAcc.html'},//后续会删除 对账
	[PayTypeData.DSS_TYPE] : {url:'yqt/pay/billPayment.html'},//票据支付
	[PayTypeData.DSS_TEMPLET_TYPE] : {url:'yqt/pay/billPayment.html'},//安全硬件模板
	[PayTypeData.CREDCARD_TYPE] : {url:'yqt/pay/credcard_issue.html'},//结算卡授权
	[PayTypeData.DIGITALCHECK_TYPE] : {url:'yqt/pay/digital_issue.html'},//数字支票签发授权
	[PayTypeData.BATCHTRANSFER_TYPE] : {url:'yqt/pay/batchTransfer.html'}//批量转账
};
...
PayHandler.prototype = {
	...
	genFlow:function(url,param,payType,vm){
		var url = url || 'yqt/pay/applyAndApprove.html';
	...
	genSignData:function(flowData,payType,vm){
		...
		}else if(payType==PayTypeData.DSS_TEMPLET_TYPE){//安全硬件模板签名
	...
	genVerifyData:function(dataSign,payType,vm){
		...
		}else if(payType==PayTypeData.DSS_TEMPLET_TYPE){//安全硬件模板
		...
		dssBbShowBeanMap[payType] = dssBbShowBeanMap[payType] || dssBbShowBeanMap.defaultDssShow;//安全硬件显示内容
	...
	openPswDialog:function(payType){
		...
		if(openPswFlag){//打开安全硬件输入框
			//return DSSHandler.openPswDialog();
            return new Promise(function(res){
                res('123456');//屏蔽安全硬件输入框
            })
		}
		return new Promise(function(res){
			res(1);//不打开安全硬件输入框
	...
	initDSSManagment:function(loginType){
		...
		return DSSHandler.ConnectAndLoginFunction(loginType).then(function(data){//①判断安全硬件是否初始化成功，计算密码需要登录，传1
	 ...
	 */
	getPubAcc:function(){
		return cryptPost('bank/getAccountList.html', {});
	}
