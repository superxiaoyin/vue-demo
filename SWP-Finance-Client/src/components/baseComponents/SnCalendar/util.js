...
export function zeroComplete(n) {
    return n < 10 ? '0' + n : `${n}`
}
...
export function getDays({year, month, value, rangeBegin, rangeEnd, returnSixRows = false}) {
    ...
    for (i = 1; i <= lastDateOfMonth; i++) {
        ...
        if (dow === 0) {
            ...
            for (let j = 0; j < firstDayOfMonth; j++) {
                ...
                temp[line].push({
                    year: rs.year,
                    month: rs.month,
                    month_str: zeroComplete(rs.month + 1),
                    day: k,
                    isCurrent: false,
                    isLastMonth: true
                });
        ...
        let _format = year + '/' + zeroComplete(month + 1) + '/' + zeroComplete(i);
        let options = {
            year: year,
            month: month,
            month_str: zeroComplete(month + 1),
            day: i,
            isCurrent: (value && dateFormat(new Date(value), 'YYYY/MM/DD')) === _format,
            isToday: dateFormat(new Date(), 'YYYY/MM/DD') === _format,
            isCurMonth: true
        };
    ...
    return {
        ...
        days: temp.map(line => {
            line.map((item, index) => {
                ...
                item.formatedDate = `${item.year}/${item.month_str}/${zeroComplete(item.date)}`;
                return item;
            });
            return line;
        })
