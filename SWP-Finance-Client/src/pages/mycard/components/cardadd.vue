...
<script>
import {openRouterByValue,backRouterByValue, getServerData,cardError} from '../../handler/handler'
...
export default {
    ...
    methods: {
            ...
            registerHandler('notifyAppBack', function (data) {//点击app返回事件
                throttle(function () {
                    if (_this.selectFlag.show) {
                        ...
                        backRouterByValue('accountadd',_this.$route.query,_this);
                    }else{
        ...
        submitOpen() {
            ...
            if(!!_this.$route.query.cardData){//加挂一类户
                _this.bindFirstCard();
        ...
        async bindFirstCard(){
            ...
            const bindData = {
                ...
                phone: _this.content.phone,
                accountName: _this.content.name,
                vCode: _this.content.vCode
            }
            try {
                _this.loadingFlag = true;
                const bindResult = await getServerData('accountMgr/accountMgr.bindFirstCard',bindData,false);
        ...
        confirm() {
            ...
            _this.loadingFlag = true;
            getServerData('salaryCard/salaryCard.add',data).then(res=>{
                _this.loadingFlag = false;
                if(res.code === 0){
                    if(!!res.cardIdList){
                        openRouterByValue('result',{result:encodeURIComponent(JSON.stringify({status:1,text:'添加成功',desc:'您可以使用银行卡进行支付',backRouter:{name:'cardlist'}}))},_this);
                    }else if(!!res.failList&&res.failList.length){
                        const errorInfo = cardError[res.failList[0].status];
                        openRouterByValue('result',{result:encodeURIComponent(JSON.stringify({status:2,text:'添加失败',desc: errorInfo}))},_this);
                    }                    
                }
            },rej=>{
                _this.loadingFlag = false;
        ...
        getendingfun(data) {
            let _this = this;
            if (data) {
                getServerData('user/user.getVerifyCode',{userPhone :_this.content.phone,vType:_this.vtype}).then(res => {
                    if (res.code ===0) {
