    ...
    methods:{
        ...
        init(){
            ...
            registerHandler('notifyAppBack', function (data) {//点击app返回事件
                throttle(function () {
                    const routerName = !!_this.$route.query.addData?'accountadd':'supercardlist';
                    backRouterByValue(routerName,_this.$route.query,_this);
                }.bind(this));
            });
        ...
        async untyingFirstCard(){
            ...
            try {
                _this.loadingFlag = true;
                const untyingResult = await getServerData('accountMgr/accountMgr.untyingFirstCard',untyingData);
                _this.loadingFlag = false;
                let data = {};
        ...
        getendingfun(data) {
            let _this = this;
            if (data) {
                getServerData('user/user.getVerifyCode',{userPhone :_this.cardData.phone,vType:_this.vtype}).then(res => {
                    if (res.code ===0) {
