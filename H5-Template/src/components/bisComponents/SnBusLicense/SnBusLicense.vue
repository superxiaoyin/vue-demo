<template>
	<div>
	    ...
	    <div class="sn-bus-license-body" :data-num='datanum'>
			<div class="sn-bus-license-list">
				<ul v-if="previewAttachList.length>0">
					<li class="cursor_pointer" v-for="(item,index) in previewAttachList" :key="index"  @click="downLoadFileReq(item,index) " >
						<span class="sn-bus-license-photo"><!--sn-bus-licenseClass：附件不同类型默认样式-->
						...
						<span class="sn-bus-license-name" :title='item.fullfileName'>{{item.fileName}}</span>
						<div :class="item.statusClass"></div><!--statusClass：附件上传过程中的状态样式-->
						<div v-if='item.delFg' class="sn-bus-license-photo-del" @click.stop='delImg(item,index);'></div><!--delFg：删除按钮是否显示-->
						<div v-if='isPC' class="sn-bus-license-redownload" @click.stop='reDownload(item,index);'></div><!--pc版本另存为按钮-->
					</li>
				</ul> 
                <ul v-if="selFlg=='1'">
                    <li class="cursor_pointer" @click.stop="showDialog">
                        <span class="sn-bus-license-photo"><!--sn-bus-licenseClass：附件不同类型默认样式-->
                            <img src="../resource/img/bis_lisence.png"> 
                        </span>
                        <span class="sn-bus-license-name">示例图</span>
                        <div v-if='isPC' class="sn-bus-license-redownload" @click.stop="showDialog"></div><!--pc版本另存为按钮-->
                        <div v-transfer-dom>
                             <scroll-lock
                                class="popup-con"
                                :lock="true"
                                :bodyLock="showExample"
                            >
                                <div class="dialog" v-if="showExample">
                                    <div class="img-example">
                                        <div class="closeBut" @click="closeDialog"></div>
                                        <img src="../resource/img/bis_lisence.png"> 
                                        <SnButton @SnButtonClick="closeDialog">我知道了</SnButton>
                                    </div>
                                </div>
                            </scroll-lock>
                        </div>
...
<script>
    ...
    import SnButton from "../../baseComponents/SnButton/SnButton.vue";
    ...
    export default {
        ...
        mounted:function(){//组件渲染生成dom
        	var _this = this;
            console.log("SnBusLicense  mounted==============", _this.previewAttachList);
        ...
        watch:{
            ...
            previewAttachList: {
                immediate:true,
                handler:function(){
                    console.log(this.previewAttachList);
                }
            }
        },
        methods: {
			...
			delImg:function(item, index){
				...
				if (!!item.fileId) {//如果文件已经上传成功，则需要删除seUploadFileList中的对象
                    ...
                    if(_this.seUploadFileList.length == 0){
                        this.$emit('seUploadFileList', "");//content需要的传递给服务器的数据list
                    }else{
                        this.$emit('seUploadFileList', _this.seUploadFileList);//content需要的传递给服务器的数据list
                    }	
				}
