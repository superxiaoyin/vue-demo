            ...
            <div class="wh_content_item" v-for="(item,index) in month.daylist"
              ...
              <div>
                ...
                <li v-else="(isHideOtherday&&item.nextDayShow)||item.otherMonth||item.dayHide"
                  ...
                  <div v-if="!item.isTodayNow">{{item.id}}<span style="font-size: .2rem;color: #478aee"
...
//            this.createYearData();
    ...
    methods: {
      ...
      timeChange(time) {
        ...
        if (that.displayMode != 4 && that.displayMode != 2) {
...
//              }, 5);
	  ...
	  * */
      ...
      setDate(timeStamp) {
        ...
        //如果时间时间区间则接受array
        if (that.displayMode == 4) {
          that.startDateStr = new Date(timeStamp[0] * 1000).format("yyyy年MM月dd日");
          that.endDateStr = new Date(timeStamp[1] * 1000).format("yyyy年MM月dd日");
          that.markedRange = [new Date(new Date(timeStamp[0] * 1000).toLocaleDateString()).getTime(), new Date(new Date(timeStamp[1] * 1000).toLocaleDateString()).getTime()];
          const myData = new Date(timeStamp[0] * 1000);
          const Json = {
          ...
          const myData = new Date(timeStamp); //任意时间
          const Json = {
            'year': myData.getFullYear(),
            'month': myData.getMonth() + 1,
            'day': myData.getDate()
