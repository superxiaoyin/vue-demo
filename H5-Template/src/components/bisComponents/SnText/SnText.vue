...
import { QueryCompanyDetailFunction } from '../../../lib/common/SnJsBridge.js';
...
import SnTextarea from "../../baseComponents/SnATextarea/SnATextarea.vue";
export default {
    components: {
        Cell,
        SnTextarea
    ...
    created: function () {
        var _this = this;
        if (_this.fieldKey == 'declareCpyName') {
            var cpyId = _this.$root.cpyId;
            QueryCompanyDetailFunction(JSON.stringify({ cpyId: cpyId + "" })).then(function (cpyInfo) {
                if (0 == cpyInfo.ret) {
                    _this.value = cpyInfo.responseData.cpyName;
                }
            })
        }
    ...
    mounted: function () {//存在value,显示value  供存草稿使用
        ...
        _this.$refs.input.$refs.input.$refs.textarea.addEventListener("input", function (e) {
    ...
    methods: {
        changeValue: function () {
            ...
            setTimeout(() => {
                _this.$refs.input.$refs.input.updateAutosize();
            }, 50)
        },
        /**
         * 失去焦点时
        ...
        blurValue: function (value) {
            ...
            if (_this.fieldKey == 'pCode' && !!value && value != '') {
                ...
                cryptPost('yqt/wealth/getProdDetail.do', { 'type': 1, 'key': value }).then(function (data) {
    ...
    watch: {
        ...
        value: function (newVal, oldVal) {
            ...
            this.$refs.input.$refs.input.updateAutosize();//大写金额等赋值关联组件需主动触发自动高度xiaowe 2018-1-4
        },
    }
}
</script>