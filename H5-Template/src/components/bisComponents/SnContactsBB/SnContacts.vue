...
<script>
    ...
    import { showToast, setStorage, getStorage, getClass, isPC } from 'sslib/common/extend.js';
    import { ToastPlugin, Group, Cell, CellBox } from 'vux';
    Vue.use(ToastPlugin);
    export default {
        components: {
            Group,
        ...
        mounted: function () {//组件渲染生成dom
            ...
            if (!!_this.value) {//草稿的存在的情况
                ...
                if (_this.isInDetailFlag) {//明细
                    UANames = _this.$root.content.inDetail[_this.idx].UANames;
                } else {
                    if (_this.defaultFlag) {
                        UANames = _this.$root.content.UANames;
                    } else {//技术支持模板 市场负责人无默认值
                        UANames = _this.$root.content.contentNames;
                    }
                }
                ...
                if (_this.defaultFlag) {//如果组件存在默认值
                    UANames = [nativeInfo.UAName];
                    UAIds = [nativeInfo.UAId];
                    uPhones = [nativeInfo.uPhone];
            ...
            value: function (oldVal, newVal) { //主要供明细控件删除使用
                ...
                if (this.$parent.delInDetailFlag) {//明细控件删除
                    ...
                    if (this.$root.content.inDetail[this.idx].UANames) {
                        UAIds = this.$root.content.inDetail[this.idx].UAId;
                        if (!UAIds) {
                            UAIds = this.$root.content.inDetail[this.idx].UAName;
                        }
        ...
        methods: {
            formatData: function (UAIds, UANames, uPhones) {
            ...
            choose: function (selectType) {
                ...
                var selectJson = {
                    ...
                    dataList: [
                        ...
                        { key: 'select_model', value: _this.selectType + '', type: "string" },
            ...
            getNextUserList: function (auditModeId, currOptType) {
                ...
                cryptPost('yqt/flow/getNextUserList.do', {
