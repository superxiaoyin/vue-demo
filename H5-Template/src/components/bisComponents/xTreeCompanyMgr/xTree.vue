<template>
	<div class="treeWrap">
		<div class="treeNode" v-for="(item,index) in treeData">
			<div class="title" v-if="!!item.name" @click.stop="choosedTree(item)">
				<b v-if="!!item.children" class="hidden" :class="{show:item.expand}" @click.stop="slide(item)"></b>
			
				<i :class="{choosed:item.checked}"></i><span>{{item.name}}</span></div>
