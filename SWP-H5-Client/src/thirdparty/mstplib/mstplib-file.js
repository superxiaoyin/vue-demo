...
function MstpJsSDK() {
    ...
    this.getFileMd5 = function(nativeFile, callback) {
        ...
        var fileReader = new FileReader(),
            ...
            chunks = Math.ceil(nativeFile.size / chunkSize),
            currentChunk = 0,
        ...
        var loadNext = function() {
            ...
            var end = ((start + chunkSize) >= nativeFile.size) ? nativeFile.size : start + chunkSize;
    ...
    this.uploader = function(option) {
        ...
        // 获取文件秒传所需参数
        var get_detect_file_param = function(file) {
            var detect_file_obj = {
                token : mToken,
                fileName : file.name,
                mstpId : mMstpId
        ...
        // 获取文件直接上传所需参数
        var get_direct_upload_param = function(file) {
        ...
        // 获取文件分片上传所需参数
        var get_chunk_upload_param = function(fileId, chunkSeq, chunkSize, checkMd5) {
        ...
        // 获取合并分片所需参数
        var get_merge_file_param = function(file, chunkCount) {
            var merge_file_obj = {
        ...
        var chunkUpload = function(file) {
            ...
            var multipart_params = {
                //appId: null,
                //secretKey: null,
                params: get_chunk_upload_param(file.transactionId, chunkId, chunkSize, chunkMd5),
        ...
        uploader.bind("FilesAdded", function(uploader, files) {
            ...
            // 文件秒传
            var detectFileExistence = function(file, callback) {
                var url = that.getDomain(mFsUrl) + that.Constant.URL_FILE_DETECT;

                var detect_params = {
                    //appId: null,
                    //secretKey: null,
                    params: get_detect_file_param(file)
                };
                    ...
                    that.getFileMd5(target.getNative(), function (code, md5) {
                        ...
                        if (code === that.Constant.ErrorCode.SUCCESS) {
                            ...
                            if (that.checkExist(md5)) {
                                ...
                                detectFileExistence(target, function (isExist, fileId) {
                                    if (isExist) {
        ...
        uploader.bind("BeforeUpload", function(uploader, file) {
            ...
            var directUpload = function() {
                var multipart_params = {
                    //appId: null,
                    //secretKey: null,
                    params: get_direct_upload_param(file),
                    'Content-Type' : "text/plain"
                };
        ...
        uploader.bind("FileUploaded", function(uploader, file, result) {
            ...
            // 合并分片
            var mergeAllChunks = function(file, callback) {
                ...
                var merge_params = {
                    //appId: null,
                    //secretKey: null,
                    params: get_merge_file_param(file, chunkCount)
                };
            ...
            if (file.isDirectUpload) {
                ...
                mergeAllChunks(file, function(success, fileId) {
                    if (success) {
    ...
    this.previewUrl = function(option) {
        ...
        var detectFilePreview = function(option, callback) {
            var xhr = new moxie.xhr.XMLHttpRequest();
            ...
            var param = that.getHttpParam(preview_params);
        ...
        detectFilePreview(option, function(success, url) {
            if (success) {
                var previewUrl = that.getDomain(url) + that.Constant.URL_FILE_PREVIEW;
                var param = that.getHttpParam(preview_params);
                previewUrl += (previewUrl.indexOf("?") > 0 ? "" : "?") + param;
