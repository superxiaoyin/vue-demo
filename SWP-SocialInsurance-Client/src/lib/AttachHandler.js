...
var AttachHandler = {
	...
	formatDetailForPreview:function(attachDetailList){
        ...
        return _this.getPreviewAttachList(preList, null, attachDetailList);
	},
	/**
	 * 获取附件预览数据列表
	 * @param {Object} previewAttachList
	...
	getPreviewAttachList:function(previewAttachList, files, fileDataList, defaultIndex) {
		...
		//         		if (extensions == "IMG") {
        //                     let url = MstpFileHandler.downloadFileUrl(attachObj.fileId)
        //                         fileDetailObj['imgsrc'] = url;
		// 	    		}
		// 			}
		// 		} 
                ...
                for (var i=0; i < fileDataList.length; i++){
                    ...
                    for(var key in fileDataObj){
                        ...
                        if(attachObj.fileName){
                            ...
                            fileDetailObj['fileSize'] = attachObj.fileSize.replace(/[a-zA-Z]/g,"")*1024*1024;
                            if (extensions == "IMG") {
                                MstpFileHandler.downloadFileUrl(attachObj.fileId).then(function (url) {
                                    fileDetailObj['imgsrc'] = url;
                                    previewAttachList.push(fileDetailObj);
                                    if (i >= fileDataList.length) {
                                        console.log("fileDataList.length: ", fileDataList.length, "i: ", i);
                                        console.log(previewAttachList);
                                        res(previewAttachList)
                                    }
                                }, function (e) {
                                    console.log(e);
                                })
                            } else if (fileDetailObj) {
                                previewAttachList.push(fileDetailObj);
                                if (i >= fileDataList.length) {
                                    res(previewAttachList)
                                }
                            } else { 
                                rej("getPreviewAttachList")
                            }
                        }
                    } 
                    //如果fileDetailObj不为空则添加
