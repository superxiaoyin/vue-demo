...
function MstpJsSDK() {
    ...
    this.uploader = function(option) {
        ...
        var uploader = new plupload.Uploader({
            ...
            //     FilesAdded: function(uploader, files) {//单选，选中文件后立即执行压缩，如果压缩成功会直接上传
            //         //这边只在文件添加的时候压缩。压缩后会移除掉文件，然后再次触发压缩的功能，知道符合标准直接上传
            //         downSizeImage(uploader,files,option);
            //     },
            // }
        });
    ...
    var downSizeImage= function (uploader,files,option) {
        var $def = $.Deferred();
        uploader.stop();
        ...
        mOxieLoader.onload = function () {
            ...
            uploader.files[0].nativeFile = file.getNative();
            uploader.start()
        }
