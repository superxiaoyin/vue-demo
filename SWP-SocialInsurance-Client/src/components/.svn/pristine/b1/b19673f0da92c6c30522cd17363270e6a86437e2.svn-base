    ...
    export default {
        ...
        methods: {
        	...
        	beforeAddFunction:function(files){
                debugger
        		var _this = this;
        		_this.isNoticeUpload = localStorage.getItem('isNoticeUpload') || _this.isNoticeUpload;
        		if(!_this.isNoticeUpload){
        	...
        	addSuccessFunction:function( file, fileId){
                ...
                var _this = this;
                _this.isNoticeUpload = localStorage.getItem('isNoticeUpload') || _this.isNoticeUpload;
				localStorage.removeItem('isNoticeUpload');
                ...
                if(!_this.isNoticeUpload){
                	...
                	if(_this.isPC){
                		$('#myPcPic').next().attr('dataSrc', fileId);
	                	$('.imageChecked').removeClass('active');
	                	$('#myPcPic').attr('src', MstpConf.ipAddress + "/file/anonymousDownload?fileId=" + fileId).next().addClass('active').parent().removeAttr('style');
                	}else{
                		$('#myAppPic').next().attr('dataSrc', fileId);
		                $('.imageChecked').removeClass('active');
		                $('#myAppPic').attr('src', MstpConf.ipAddress + "/file/anonymousDownload?fileId=" + fileId).next().addClass('active').parent().removeAttr('style');
                	}
                	_this.isNoticeUpload = false;
                }
        	},
        	/**
        	 * 附件点击预览方法
