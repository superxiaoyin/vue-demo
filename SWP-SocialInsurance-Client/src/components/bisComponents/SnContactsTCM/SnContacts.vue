        ...
        <group label-width="5em">
            <cell-box class="vux-tap-active" ref="selectPerson" @click.native="choose(chooseType)">
                ...
                <div class="content_names" :placeholder='placeholder'>
                    <span v-for='(item,index) in UAIds' v-bind:class='{member_del:delFlag==true}' :key="index"
            ...
            <div class='quick_name_box'>
                <div class='tijiaogei'>
                    ...
                    <ul id='usedNextContactUL' class='tijiaogei_info_box'>
                        <li class="tijiaogei_info clearfix" v-for="(item,index) in nextUserList" :key="index"
                            @click="chooseNextUser(item)">
...
<script>
    ...
    import { OpenActionFunction, GetUserInfoFunction } from '../../../lib/common/SnJsBridge.js';
    import { cryptPost, nativeInfo } from '../../../lib/common/base.js';
    import DepartmentHandler from './DepartmentHandler.js';
    import { showToast, setStorage, getStorage, getClass, isPC } from '../../../lib/common/extend.js';
    ...
    export default {
        ...
        watch: {
            value: function (newVal, oldVal) { //主要供明细控件删除使用
                ...
                if (0 < _this.UAIds.length && '' != newVal) {
                    GetUserInfoFunction({ "UAId": parseInt(newVal) }).then(function (uaData) {
        ...
        methods: {
            ...
            getSalaryList: function (UAId) {
                var _this = this;
                //获取工资卡
                cryptPost('cpy/getSalaryCardInfo.html', { choosedUAId: UAId }).then(function (result) {
