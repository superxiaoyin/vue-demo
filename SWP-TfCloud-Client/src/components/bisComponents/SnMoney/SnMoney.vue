<template>
    <div class="overflow-show" :class="{totalMoney:totalFlag}">
        <!--金额控件对ios单独做分支，使用span解决ios重影的问题，方案不可取，导致外汇模板中使用了金额控件，并且只读，在ios平台上就会进入该分支。span上不能实现placeholder功能-->
        <cell :title="title" value-align="left" v-if="readonly && isIos && fieldKey != 'preMoney'">
            <span ref="input" :value="formatMoney" :class="{colorRed:totalFlag}">{{formatMoney}}</span>
            <em class="payByCompanyOff" v-bind:class="{payByCompanyOn:cpyPayFlag}" v-if="cpyPayShowFlag" @click="choosedCpyPay"></em>
            <!--赎回点击全部功能的实现-->
            <em class="redeemAll" v-if="redeemFlag" @click="chooseAllredemm">全部</em>
            <!--公司代付-->
            <em class="adjustFlagPlus" v-bind:class="{adjustFlagMinus:balanceMinusFlag}" v-if="balanceShowFlag" @click="choosedAdjust"></em>
            <!--银企对账调节金额-->
        </cell>
        <cell :title="title" value-align="left" v-else>
            ...
            <em class="adjustFlagPlus" v-bind:class="{adjustFlagMinus:balanceMinusFlag}" v-if="balanceShowFlag" @click="choosedAdjust"></em>
            <!--赎回点击全部功能的实现-->
            <em class="redeemAll" @click="chooseAllredemm" v-if="redeemFlag">全部</em>
            <!--银企对账调节金额-->
        </cell>
    ...
    mounted: function () {
        if (this.value) {
            ...
            if (this.value.replace(/0/g, "")) {
                ...
                if (this.fieldKey == 'buyMoney' || this.fieldKey == 'loanAmount') {
                ...
                this.$refs.input.value = "0.00元"; //申请时金额填0，再次申请时，金额未赋值
            }
        } else {
            this.setZeroValue("difference");
        }
        console.log("filename = " + this.title);
        // if(this.title == '赎回金额' ) {
        //     this.redeemFlag = true;
        //     console.log("redeemTotalMoney = "+this.redeemTotalMoney);
        // } else {
    ...
    methods: {
        ...
        inputFrtValue: function (value) {
            ...
            if ("" == result) {
                ...
                if (_this.fieldKey == 'buyMoney' || _this.fieldKey == 'loanAmount') {//北京银行POC演示 （外汇结汇、购汇模板）
                    this.$emit(
                        "input",
                        (minus + MoneyValidator.format(result)).substring(0, (minus + MoneyValidator.format(result)).length - 1).replace(/[^\d]/g, "")
                    );
                } else {
                    this.$emit(
                        "input",
            ...
            var resultMoney = MoneyValidator.format(result).replace(/[^\d]/g, "");
            if (_this.fieldKey == 'amount') {
                if ((!!_this.$root.content.transMethod && _this.$root.content.transMethod == 2 || _this.$root.subTransMethod == 2) && _this.$root.superBank && parseFloat(resultMoney) <= AmountLimit) {
            ...
            if (this.unitprice) {
                ...
                var amountValue = this.$root.$refs.inDetail[0].$refs[amountId][
                    this.idx
                ].value;
                var totalPrice = 0;
        ...
        blurFrtValue: function (value) {
            ...
            if (value && value != "无金额") {
                if (this.fieldKey == 'buyMoney' || this.fieldKey == 'loanAmount') {//北京银行POC演示 （外汇结汇、购汇模板）
                    this.$refs.input.value = MoneyValidator.format(value).substring(0, MoneyValidator.format(value).length - 1); //格式化显示数据
                    this.$emit('input', this.$refs.input.value);
                    this.$emit('change', this.$refs.input.value);
                } else {
                    if(!!this.startAmount){//存在起存金额
                        if(Number(value)<Number(this.startAmount)){//输入金额小于起存金额
                            showToast('输入金额应不小于起存金额','middle');
        ...
        genRelationValue: function (value) {
            ...
            //差额
            if (
                this.relationFiledIds &&
                "difference" == this.relationFiledIds[0]
            ) {
            ...
            if (
                this.relationFiledIds &&
                "bankRemain" == this.relationFiledIds
            ) {
            ...
            if (
                ...
                if (
                    this.summationFlag ||
                    _this.$root.templetType ==
                    TempletTypeData.TEXAPPLY_TEMPLETTYPE
                ) {
                    //累加标识  针对总计
                    ...
                    if (_this.$parent.$refs.moneyBig) {
                        ...
                        if ("无金额" == value) {
                            ...
                            moneyBig = moneyUppercase(
                                MoneyValidator.format(value).replace(
                                    /[^\d]/g,
                                    ""
                                )
                            );
                        ...
                        _this.$parent.$refs.moneyBig[
                            _this.idx
                        ].value = moneyBig;
                        _this.$root.content.inDetail[
                            _this.idx
                        ].moneyBig = moneyBig;
                    }
                } else {
        ...
        calSummation: function (obj, fieldKey) {
            ...
            //支行显示判断
            if (((!!_this.$root.content.transMethod && _this.$root.content.transMethod == 2) || _this.$root.subTransMethod == 2) && _this.$root.superBank && parseFloat(result | 0) <= AmountLimit) {
                _this.$root.interBankFlag = false;
            } else {
                _this.$root.interBankFlag = true;
            }
            ...
            if (result.length > 16) {
                ...
                if (
                    _this.templetType ==
                    TempletTypeData.BATCHTRANSFER_TEMPLETTYPE
                ) {
                    toastMsg = "转账";
                }
                    ...
                    this.$parent.inDetailList.forEach(function (item) {
                        ...
                        if (cpyFlag) {
                            addFlag =
                                item.summationFlag &&
                                obj.inDetail[i][
                                item.fieldKey + "_payByCompanyFlag"
                                ];
                        } else {
                            addFlag =
                                item.summationFlag &&
        ...
        chooseAllredemm() {
            //选中所有的金额
            console.log("select  all  Money");
            let tempValue = moneyStringFixTwo(
                (this.redeemTotalMoney + "").replace(/[^\d]/g, ""));
            this.$refs.input.value = tempValue;
            this.$emit("input", this.redeemTotalMoney);
        },
        /**
         * 选中公司代付
         */
        ...
        setZeroValue: function (fieldKey) {
            if (fieldKey == this.fieldKey && this.value !== "") {
    ...
    watch: {
        value: function (newVal, oldVal) {
            ...
            if (this.value && !this.inputFlag) {
                ...
                if (this.fieldKey == 'buyMoney' || this.fieldKey == 'loanAmount') {
                    let money = moneyStringFixTwo(
                        (this.value + "").replace(/[^\d]/g, "")
                    );
                    this.$refs.input.value = money.substring(0, money.length - 1);
                    this.formatMoney = money.substring(0, money.length - 1);
                } else if (this.fieldKey == 'preMoney') {
                    let value = Number(this.value).toFixed(2);
                    this.$refs.input.value = value + '元';
                    this.formatMoney = value + '元';
                } else {
                    this.$refs.input.value = moneyStringFixTwo(
                        (this.value + "").replace(/[^\d]/g, "")
                    );
