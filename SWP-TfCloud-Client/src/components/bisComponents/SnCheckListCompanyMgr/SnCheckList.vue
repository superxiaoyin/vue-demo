<template>
	<div class="SnCheckBox">
        <cell :title="title" value-align="left" class="snPopupRight"  @click.native="openPopup" >
        	<input type="text" class="popupInput" style='width:97%' maxlength="20" :placeholder="placeholder" readonly="readonly" ref="input" :value='selectedValue' />
        </cell>        
        <popup  v-model="popupShowFlag" position="right" width="100%">
            <div class="position-horizontal-demo popupDebit" v-if="popupShowFlag">
            	<checklist label-position="left" :max='maxNumber' :options="dataList" v-model="checklist"></checklist>
            </div>
			<div v-transfer-dom class="checksubmitButWrap">
				<div class="checksubmitBut" v-show="popupShowFlag">
					<slot name="btn"></slot>
					<div class="submitBut_c" @click="choosedList()">确认</div>
				</div>
			</div> 
        </popup>
    </div>
...
<script>
	...
	import {Popup,PopupHeader,Cell,Checklist,TransferDom} from 'vux';
    export default {
		directives: {
			TransferDom
		},
