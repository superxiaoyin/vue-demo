        ...
        <div class="wh_content_all">
            <div class="content_tabs">
                <div
                    ...
                    @click="cancel"
                    v-show="onlyTime"
                    v-bind:class="{ onlyTime: onlyTime }"
                >
                    取消
                </div>
                <tab v-show="!onlyTime && !dateRange" class="newTab">
                    <tab-item
                        selected
                        @on-item-click="switchSelect('date')"
                        v-if="!dateRange"
                        class="tabItem"
                        >{{ dateTop }}
                    </tab-item>
                    <tab-item
                        @on-item-click="switchSelect('time')"
                        v-if="!onlyDate && !dateRange && displayMode < 5"
                        class="tabItem"
                        >{{ selectedTime }}
                    </tab-item>
                </tab>
                <tab v-show="dateRange" class="rangeTab">
                    <tab-item
                        selected
                        @on-item-click="switchSelect('startDate')"
                        class="tabItem"
                        >{{ startDateStr }}
                    </tab-item>
                    <span style="line-height: .9rem">—</span>
                    <tab-item
                        @on-item-click="switchSelect('endDate')"
                        class="tabItem"
                        >{{ endDateStr }}
                    </tab-item>
                </tab>
            ...
            >
                <swiper-item v-for="year in yearList" :key="year.year">
                    <div class="yearSelect_content">
                        <div
                            class="yearSelect_content_item"
                            v-for="(item, index) in year.monthList"
                        ...
                        >
                            <div>
                                <li
                                    ...
                                    v-if="
                                        (isHideOtherday && item.nextDayShow) ||
                                            item.hidden
                                    "
                                    v-bind:class="{
                ...
                <swiper-item v-for="month in swiperList" :key="month.timestamp">
                    <div class="wh_content" @touchmove="measureStart($event)">
                        <div
                            ...
                            v-for="(item, index) in month.daylist"
                            @click="clickDay(item, index, true)"
                        >
                            <div>
                                <div
                                    class="pointBlockLeft"
                                    v-if="
                                        dateRange &&
                                            item.isEndDay &&
                                            (markedRange[0] != markedRange[1]) &
                                                (markedRange[0] != 0) &&
                                            item.otherMonth == 0
                                    "
                                    v-bind:class="{}"
                                ...
                                <li
                                    ...
                                    "
                                    v-bind:class="{
                                        isToday_now: item.isNow,
                                        hideRange: item.isBetween,
                                        hidePoint:
                                            item.isStartDay || item.isEndDay
                                    }"
                                >
                                    {{ item.id }}
                                </li>
                                <li
                                    ...
                                    v-bind:class="{
                                        wh_isToday:
                                            (item.isToday || item.isTodayNow) &&
                                            !item.isBetween,
                                        isRangeSelectedPoint:
                                            item.isStartDay || item.isEndDay,
                                        isRangeSelectedBetween: item.isBetween
                                    }"
                                >
                                    <div>
                                        {{ item.id
                                ...
                                <div
                                    ...
                                    v-bind:class="{
                                        ...
                                        isToday: item.isToday && item.isMark
                                    }"
                                ></div>
                            </div>
                        </div>
...
import {
Popover, Tab, TabItem, Swiper, Picker,SwiperItem,
} from 'vux'
export default {
    name: 'calendar',
    components: {
        ...
        * */
        createTime(timeStamp) {
            ...
            for (let i = 0; i < 24; i++) {
                ...
                //        hours.push(i.toString() + '时')  //bug 26751 fengz
            }
            for (let i = 0; i < 60; i++) {
                minutes.push(i.toString() + '')
                //        minutes.push(i.toString() + '分')  //bug 26751 fengz
            }
        ...
        createDate(chooseMonth, type) {
            ...
            if (!!chooseMonth) {
                ...
                if (isChooseMonth) {
                    array.push({ 'daylist': that.getList(chooseDay, false), 'Date': chooseDay });      //bug 26758 fengz
                } else {
                    array.push({ 'daylist': that.getList(chooseDay, chooseDay), 'Date': chooseDay });  //bug 26758 fengz
                }
        ...
        commitDate() {
            const that = this;
            if (that.displayMode == 5) {
                const str = that.dateTop;
                const chooseMonth = new Date(str.slice(0, 4), str.slice(5, 7) - 1, 1);
                const date = {
                    'date': new Date(chooseMonth.getTime()),
                    'time': '',
                    'timeStamp': chooseMonth.getTime() / 1000
                };
                ...
                if (!(that.isHideOtherday && item.nextDayShow) && !item.dayHide && that.swiperList.length > 0) {
                    ...
                    for (let i = 0; i < that.swiperList[that.swiperIndex].daylist.length; i++) {
                        if (that.dateRange && type) {
                            ...
                            //清除当前日期的选中 fengz
                            if (that.swiperList[that.swiperIndex].daylist[i].isTodayNow) {
                                that.swiperList[that.swiperIndex].daylist[i].isTodayNow = false;
                            }
                        }
                    }
